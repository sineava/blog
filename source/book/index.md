---
title: 编程书籍推荐
cover: 'https://testingcf.jsdelivr.net/gh/sineava/picture-bed//cover/bg-18.jpg'
background: url(https://testingcf.jsdelivr.net/gh/sineava/picture-bed//cover/bg-18.jpg)
comments: true
date: 2022-10-26 23:17:10
updated: 2022-10-26 23:17:10
---

## 前端

| 书名 | 描述 | 豆瓣评分 |
| :------ | :-----: | ------: |
| JavaScript权威指南<第七版> | JavaScript语言核心 | 9.6 |
| Vue.js设计与实现 | Vue3功能模块源码解析 | 9.6 |
| CSS揭秘 | CSS 技巧 | 9.3 |
| 深入理解ES6 | ES6 | 9.3 |
| JavaScript设计模式与开发实践 | 设计模式 | 9.0 |
| 你不知道的JavaScript | 深入理解语言内部的机制 | 8.7 |
| JavaScript高级程序设计<第四版> | JavaScript的基础特性和高级特性 | 8.6 |

## 算法

| 书名 | 描述 | 豆瓣评分 |
| :------ | :-----: | ------: |
| 算法<第4版> | 算法和数据结构 | 9.4 |

## HTTP

| 书名 | 描述 | 豆瓣评分 |
| :------ | :-----: | ------: |
| HTTP权威指南 | 网络协议 | 8.6 |
| 图解HTTP | HTTP协议 | 8.1 |

## Linux

| 书名 | 描述 | 豆瓣评分 |
| :------ | :-----: | ------: |
| 鸟哥的Linux私房菜<第4版> | Linux操作系统 | 8.4 |